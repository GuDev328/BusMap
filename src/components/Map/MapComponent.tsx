...
const RouteMap = () => {
  ...
  return (
    <div>
      ...
      {/* Các ô nhập cho địa điểm */}
      <div style={{ marginBottom: '10px' }}>
        <Select
          showSearch
          options={locationsFrom}
          value={
            startLocation ? getLocationLabelFrom(startLocation) : undefined
          }
          placeholder="Chọn điểm bắt đầu"
          notFoundContent={loading ? <Spin size="small" /> : null}
          onSearch={debouncedGetLocationSuggestionsFrom} // Gọi hàm tìm kiếm địa điểm với debounce
          onSelect={(value) => {
            console.log(value);
      ...
      <div style={{ marginBottom: '10px' }}>
        <Select
          showSearch
          options={locationsMid}
          value={midLocation ? getLocationLabelMid(midLocation) : undefined}
          placeholder="Chọn điểm trung gian"
          notFoundContent={loading ? <Spin size="small" /> : null}
          onSearch={debouncedGetLocationSuggestionsMid} // Gọi hàm tìm kiếm địa điểm với debounce
          onSelect={(value) => {
            console.log(value);
            setMidLocation(value); // Cập nhật endLocation với tọa độ
          }} // Cập nhật endLocation
          filterOption={false} // Tắt filter mặc định của Select
        ></Select>
      </div>
      ...
      <div style={{ marginBottom: '10px' }}>
        <Select
          showSearch
          options={locationsTo}
          value={endLocation ? getLocationLabelTo(endLocation) : undefined}
          placeholder="Chọn điểm kết thúc"
          notFoundContent={loading ? <Spin size="small" /> : null}
          onSearch={debouncedGetLocationSuggestionsTo} // Gọi hàm tìm kiếm địa điểm với debounce
          onSelect={(value) => {
            console.log(value);
            setEndLocation(value); // Cập nhật endLocation với tọa độ
          }} // Cập nhật endLocation
          filterOption={false} // Tắt filter mặc định của Select
        ></Select>
      </div>
